...
const ReceitasPageExample = () => {
  ...
  const handleSaveIncome = async (income: Omit<Income, "id" | "createdAt" | "updatedAt">) => {
    try {
  ...
  return (
    <div>
      ...
      {createIncomeMutation.isPending && (
        <div>Salvando receita...</div>
      )}
      
      {deleteTransactionMutation.isPending && (
        <div>Deletando receita...</div>
      )}
    </div>
  );
};
...
const DashboardButtonExample = () => {
  ...
  return (
    <button onClick={handleQuickIncome} disabled={createIncomeMutation.isPending}>
