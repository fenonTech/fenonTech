import React, { useState } from "react";
import "./HeaderMobile.css";
import logoNavbar from "../../assets/logo.png";

interface HeaderMobileProps {
  userName: string;
  userPhoto?: string;
  onConfigClick: () => void;
  onLogoutClick: () => void;
}

const HeaderMobile: React.FC<HeaderMobileProps> = ({
  userName,
  userPhoto,
  onConfigClick,
  onLogoutClick,
}) => {
  const [showDropdown, setShowDropdown] = useState(false);

  const toggleDropdown = () => {
    setShowDropdown(!showDropdown);
  };

  const handleConfigClick = () => {
    setShowDropdown(false);
    onConfigClick();
  };

  const handleLogoutClick = () => {
    setShowDropdown(false);
    onLogoutClick();
  };

  return (
    <header className="header-mobile">
      <div className="header-mobile-logo">
        <img src={logoNavbar} alt="FenonTech Logo" className="logo-navbar" />
      </div>

      <div className="header-mobile-user">
        <button
          className="header-mobile-user-button"
          onClick={toggleDropdown}
          aria-label="Menu do usuário"
        >
          <div className="header-mobile-user-info">
            <span className="header-mobile-user-name">{userName}</span>
            <div className="header-mobile-user-avatar">
              {userPhoto ? (
                <img src={userPhoto} alt={userName} />
              ) : (
                <div className="header-mobile-user-avatar-placeholder">
                  {userName.charAt(0).toUpperCase()}
                </div>
              )}
            </div>
          </div>
          <svg
            className={`header-mobile-dropdown-icon ${
              showDropdown ? "open" : ""
            }`}
            width="16"
            height="16"
            viewBox="0 0 24 24"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M7 10L12 15L17 10"
              stroke="currentColor"
              strokeWidth="2"
              strokeLinecap="round"
              strokeLinejoin="round"
            />
          </svg>
        </button>

        {showDropdown && (
          <>
            <div
              className="header-mobile-dropdown-overlay"
              onClick={() => setShowDropdown(false)}
            />
            <div className="header-mobile-dropdown">
              <button
                className="header-mobile-dropdown-item"
                onClick={handleConfigClick}
              >
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 24 24"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M12 15a3 3 0 100-6 3 3 0 000 6z"
                    stroke="currentColor"
                    strokeWidth="2"
                  />
                  <path
                    d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 010 2.83 2 2 0 01-2.83 0l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 01-4 0v-.09A1.65 1.65 0 009 19.4a1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 01-2.83 0 2 2 0 010-2.83l.06-.06a1.65 1.65 0 00.33-1.82 1.65 1.65 0 00-1.51-1H3a2 2 0 010-4h.09A1.65 1.65 0 004.6 9a1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 010-2.83 2 2 0 012.83 0l.06.06a1.65 1.65 0 001.82.33H9a1.65 1.65 0 001-1.51V3a2 2 0 014 0v.09a1.65 1.65 0 001 1.51 1.65 1.65 0 001.82-.33l.06-.06a2 2 0 012.83 0 2 2 0 010 2.83l-.06.06a1.65 1.65 0 00-.33 1.82V9a1.65 1.65 0 001.51 1H21a2 2 0 010 4h-.09a1.65 1.65 0 00-1.51 1z"
                    stroke="currentColor"
                    strokeWidth="2"
                  />
                </svg>
                <span>Configurações</span>
              </button>
              <button
                className="header-mobile-dropdown-item logout"
                onClick={handleLogoutClick}
              >
                <svg
                  width="20"
                  height="20"
                  viewBox="0 0 24 24"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4M16 17l5-5-5-5M21 12H9"
                    stroke="currentColor"
                    strokeWidth="2"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  />
                </svg>
                <span>Sair</span>
              </button>
            </div>
          </>
        )}
      </div>
    </header>
  );
};

export default HeaderMobile;
